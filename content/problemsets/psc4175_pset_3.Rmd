...
title: "Problem Set 3"
author: '[YOUR NAME]'
date: "Due Date: 2025-10-31"
output:
  html_document: default
  pdf_document: default
subtitle: Uncertainty and Confidence Intervals
...
# Part 1: New York/Villanova Knicks (3 points; 0.5 extra credit points)

## Question 0
Require `tidyverse` and load the [`game_summary.rds`](https://github.com/rweldzius/PSC4175/raw/main/static/data/game_summary.Rds?raw=true') data to an object called `games`. (Note, you must load the data from your local computer, not from the link)
```{r}
# INSERT CODE HERE
```
...
Now recreate the same plot for the 2018, 2019, and combined seasons. Imagine that you work for the Knicks organization and Gersson Rosas (the GM), asks you if the team scores more points at home or away? Based on your analysis, what would you tell him?
...
Re-do this analysis for three other statistics of interest to Gersson: total rebounds (treb), turnovers (tov), and field goal percent (pctFG). Do you notice anything strange in these results? What might explain it?
...
# Part 2: Youtube Bias (4 points; 1.5 extra credit points)

We will be using a new dataset called `youtube_individual.rds` which can be found on the course [github page](https://github.com/rweldzius/PSC4175/raw/main/static/data/youtube_individual.rds). The codebook for this dataset is produced below. All ideology measures are coded such that negative values indicate more liberal content and positive values indicate more conservative content.
...
## Question 0
*Require `tidyverse` and load the [`youtube_individual.rds`](https://github.com/rweldzius/PSC4175/raw/main/static/data/youtube_individual.rds) data to an object called `yt`.*
...
## Question 5 [1.25 points + 0.5 EC points]
...
*Now create two univariate visualization of these errors. Based on this result, which model looks better? Why? EC [+0.5 point]: Plot both errors on the same graph using `pivot_longer()`.*
...
*Finally, create a multivariate visualization of both sets of errors, comparing them against the $X$ variable. Based on this result, which model looks better? Why? EC [+0.5 point]: Create two plots side-by-side using facet_wrap(). This is SUPER HARD, so don't worry if you can't get it.*
...
# EC [0.5 point]: Try to create two plots side-by-side. (SUPER HARD)
