...
```{r, eval=FALSE}
getwd()
setwd("~/Library/CloudStorage/Dropbox/Villanova/TEACHING/PSC4175 - Data Science/PSC4175/static/homeworks")
```
...
The first part of the code is defining a new object called `mi_ep` (short for Michigan exit poll) and then assigning (`<-`) that object to be defined by the dataset being read in by the `read_rds` function. The filepath within `read_rds` starts with ".." which tells R to go back one folder from your current working directory and then find the folder `data`, and finally read in the `MI2020_ExitPoll_small.rds` file. If you haven't saved the `MI2020_ExitPoll_small.rds` to your `data` folder and set your working directory (as explained above) then this chunk of code won't work. Go back and make adjustments as needed. 
